        ...
        if data is not None and not data['error'] and len(data['results']['list']) > 0:
            ...
            for cog in data['results']['list']:
                embed = discord.Embed(title=cog['name'],
                                      ...
                                      description=((cog['description'] and len(
                                          cog['description']) > 175 and '{}...'.format(cog['description'][:175])) or
                                                   cog['description']) or cog['short'],
                                      color=0xfd0000)
                ...
                embed.add_field(name='Command to add repo',
                                value='{}cog repo add {} {}'.format(ctx.prefix, cog['repo']['name'],
                                                                    cog['links']['github']['repo']),
                                inline=False)
                ...
                embed.set_footer(text='{}{}'.format('{} ⭐ - '.format(cog['votes']),
                                                    (len(cog['tags'] or []) > 0 and '🔖 {}'.format(
                                                        ', '.join(cog['tags']))) or 'No tags set 😢'
                                                    ))
                embeds.append(embed)
